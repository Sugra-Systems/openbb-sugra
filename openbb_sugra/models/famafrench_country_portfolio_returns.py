"""Sugra Fama-French Country Portfolio Returns Model."""
...
from openbb_famafrench.models.country_portfolio_returns import (
    FamaFrenchCountryPortfolioReturnsData,
...
class SugraFamaFrenchCountryPortfolioReturnsFetcher(
    Fetcher[
        FamaFrenchCountryPortfolioReturnsQueryParams,
        list[FamaFrenchCountryPortfolioReturnsData],
    ]
):
    """Fetch Fama-French international country portfolio returns from the Sugra API."""
    ...
    def transform_query(
        ...
        return FamaFrenchCountryPortfolioReturnsQueryParams(**params)
    ...
    async def aextract_data(
        ...
        **kwargs: Any,
        ...
        """Return the wide country portfolio records from the Sugra API."""
        # pylint: disable=import-outside-toplevel
        from openbb_sugra.utils.helpers import envelope_data, get_api_key, sugra_get

        api_key = get_api_key(credentials)
        response = await sugra_get(
            "/api/v1/fama-french/country-portfolio",
            api_key,
            {
    ...
    def transform_data(
        ...
        **kwargs: Any,
        ...
        if not data:
            raise EmptyDataError("No Fama-French country portfolio records returned.")

        start = query.start_date
        end = query.end_date

        rows: list[FamaFrenchCountryPortfolioReturnsData] = []
        for rec in data:
            ...
            try:
                rows.append(
                    FamaFrenchCountryPortfolioReturnsData.model_validate(
                        {**rec, "date": iso}
                    )
                )
        ...
        if not rows:
            raise EmptyDataError(
                "No Fama-French country portfolio records matched the query."
            )
